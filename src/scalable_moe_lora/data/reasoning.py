...
Eval-only benchmarks (OOD, loaded via --eval_datasets flag):
...
from scalable_moe_lora.data.nlg import (
    CausalLMDataset,
    tokenize_dataset,
    format_e2e,
    format_samsum,
...
def _load_mbpp(split):
    ...
    # MBPP test is not in the OOD OOD set, so there is no contamination concern.
    from datasets import concatenate_datasets
...
def load_raw_dataset(dataset_name, split="test"):
    ...
    if dataset_name in {"e2e", "samsum"}:
        from scalable_moe_lora.data.nlg import load_raw_dataset as load_raw_nlg
