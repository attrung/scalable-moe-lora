...
from src.evaluate import generate_predictions, compute_metrics as compute_nlg_metrics
from src.model import build_model
from src.data_reasoning import REASONING_DATASET_LOADERS, load_raw_dataset
from src.utils import load_config, load_checkpoint, set_seed
from src.lora_layers import MoELoRA, RoutedLoRA
...
def collect_routing_stats(model, tokenizer, inputs, device, max_samples=200):
    ...
    for name, module in model.named_modules():
        if isinstance(module, (MoELoRA, RoutedLoRA)):
...
def evaluate_all_datasets(config_path, checkpoint_path, dataset_names, seed=42,
    ...
    for ds_name in full_eval_list:
        ...
        if routing_analysis:
            ...
            if indices:
                first_routed = next(
                    (m for m in model.modules() if isinstance(m, RoutedLoRA)), None
                )
                if first_routed:
